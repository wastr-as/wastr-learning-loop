...
- **Is the first scalable demand segment occasional DIY homeowners rather than builders/transporters?** Customer-discovery ([#58](https://github.com/wastr-as/wastr-learning-loop/issues/58)) flags a third segment the loop had not modelled — first-time / occasional homeowners whose pain is *uncertainty*, not collection. Whether their friction is high enough to justify a guided, simplicity-first wedge (and whether the marketplace is a Phase-3 outcome rather than the start) is an open bet ([#59](https://github.com/wastr-as/wastr-learning-loop/issues/59)), pending a Greater-Oslo validation round (15 DIY homeowners / 10 contractors / 5 collectors). Not yet reflected in `docs/strategy/product-thesis.md` by design — it stays `learning: hypothesis` until validated.
